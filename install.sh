...
repoDir="https://raw.githubusercontent.com/playboye/vpn/main/"
...
net.ipv6.conf.lo.disable_ipv6 = 1" > /etc/sysctl.d/boyevpn.conf
sysctl -p /etc/sysctl.d/boyevpn.conf
...
net.ipv4.tcp_congestion_control = bbr" >> /etc/sysctl.d/boyevpn.conf
sysctl -p /etc/sysctl.d/boyevpn.conf
...
rm -rf dnstt
mkdir /boyevpn/dnstt
dnstt-server -gen-key -privkey-file /boyevpn/dnstt/server.key -pubkey-file /boyevpn/dnstt/server.pub
...
screen -AmdS dnstt dnstt-server -udp :5300 -privkey-file /boyevpn/dnstt/server.key ns-${installSubDomain} 127.0.0.1:85

# Install UDP Custom (https://t.me/ePro_Dev_Team)
mkdir /boyevpn/udp-custom
wget -O /boyevpn/udp-custom/udp-custom "${repoDir}files/udp-custom/udp-custom"
wget -O /boyevpn/udp-custom/config.json "${repoDir}files/udp-custom/config.json"
chmod +x /boyevpn/udp-custom/udp-custom
...
deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/trusted.gpg.d/openvpn-public-repo.gpg] https://build.openvpn.net/debian/openvpn/stable ${VERSION_CODENAME} main" > /etc/apt/sources.list.d/boyevpn.list
apt update
...
echo -e "\n# IPv4 forwarding
net.ipv4.ip_forward = 1" >> /etc/sysctl.d/boyevpn.conf
sysctl -p /etc/sysctl.d/boyevpn.conf
...
sleep 2
mkdir /boyevpn/openvpn
wget -O /boyevpn/openvpn/udp.ovpn "${repoDir}files/openvpn/client-udp.ovpn"
wget -O /boyevpn/openvpn/tcp.ovpn "${repoDir}files/openvpn/client-tcp.ovpn"
sed -i "s/install_ip/$installIP/g" /boyevpn/openvpn/udp.ovpn
sed -i "s/install_ip/$installIP/g" /boyevpn/openvpn/tcp.ovpn
echo -e "\n<ca>" >> /boyevpn/openvpn/udp.ovpn
cat "/etc/openvpn/key/ca.crt" >> /boyevpn/openvpn/udp.ovpn
echo "</ca>" >> /boyevpn/openvpn/udp.ovpn
echo -e "\n<ca>" >> /boyevpn/openvpn/tcp.ovpn
cat "/etc/openvpn/key/ca.crt" >> /boyevpn/openvpn/tcp.ovpn
echo "</ca>" >> /boyevpn/openvpn/tcp.ovpn
...
cd
rm -rf Python-2.7.18
mkdir /boyevpn/websocket
wget -O /boyevpn/websocket/ws-dropbear.py "${repoDir}files/websocket/ws-dropbear.py"
wget -O /boyevpn/websocket/ws-openvpn.py "${repoDir}files/websocket/ws-openvpn.py"
wget -O /boyevpn/websocket/ohpserver "${repoDir}files/websocket/ohpserver"
chmod +x /boyevpn/websocket/ohpserver
...
if [ "$installDNS" = "cf" ]; then
	...
	dns_cloudflare_api_token =  ${installCFToken}" > /boyevpn/xray/cloudflare.ini
	chmod 600 /boyevpn/xray/cloudflare.ini
	certbot certonly --dns-cloudflare --dns-cloudflare-credentials /boyevpn/xray/cloudflare.ini --dns-cloudflare-propagation-seconds 30 --noninteractive --agree-tos -d $installSubDomain -d *.$installSubDomain --register-unsafely-without-email
else
    snap install certbot-dns-digitalocean
	echo "# DigitalOcean API token used by Certbot
	dns_digitalocean_token = ${installDOToken}" > /boyevpn/xray/digitalocean.ini
	chmod 600 /boyevpn/xray/digitalocean.ini
	certbot certonly --dns-digitalocean --dns-digitalocean-credentials /boyevpn/xray/digitalocean.ini --dns-digitalocean-propagation-seconds 30 --noninteractive --agree-tos -d $installSubDomain -d *.$installSubDomain --register-unsafely-without-email
fi
...
deb [signed-by=/etc/apt/trusted.gpg.d/nginx-official-repo.gpg] http://nginx.org/packages/debian ${VERSION_CODENAME} nginx" >> /etc/apt/sources.list.d/boyevpn.list
apt update
apt install -y nginx
...
htpasswd -bc /etc/nginx/conf.d/.htpasswd boyevpn P4s5W0rD
...
iptables-save > /boyevpn/iptables.rules
systemctl start wg-quick@wg0
...
installIP=\"${installIP}\"
netInt=\"${netInt}\"" > /boyevpn/params

# Configure menu
echo "[INFO] Configure menu"
sleep 2
mkdir /boyevpn/log
mkdir -p /boyevpn/menu/{ssh,wireguard,xray,other}
wget -O /boyevpn/menu/ssh/add.sh "${repoDir}files/menu/ssh/add.sh"
wget -O /boyevpn/menu/ssh/delete.sh "${repoDir}files/menu/ssh/delete.sh"
wget -O /boyevpn/menu/ssh/extend.sh "${repoDir}files/menu/ssh/extend.sh"
wget -O /boyevpn/menu/ssh/list.sh "${repoDir}files/menu/ssh/list.sh"
wget -O /boyevpn/menu/ssh/detail.sh "${repoDir}files/menu/ssh/detail.sh"
wget -O /boyevpn/menu/ssh/dropbear_monitor.sh "${repoDir}files/menu/ssh/dropbear_monitor.sh"
wget -O /boyevpn/menu/ssh/openvpn_monitor.sh "${repoDir}files/menu/ssh/openvpn_monitor.sh"
wget -O /boyevpn/menu/wireguard/add.sh "${repoDir}files/menu/wireguard/add.sh"
wget -O /boyevpn/menu/wireguard/delete.sh "${repoDir}files/menu/wireguard/delete.sh"
wget -O /boyevpn/menu/wireguard/extend.sh "${repoDir}files/menu/wireguard/extend.sh"
wget -O /boyevpn/menu/wireguard/list.sh "${repoDir}files/menu/wireguard/list.sh"
wget -O /boyevpn/menu/wireguard/detail.sh "${repoDir}files/menu/wireguard/detail.sh"
wget -O /boyevpn/menu/xray/trojan_add.sh "${repoDir}files/menu/xray/trojan_add.sh"
wget -O /boyevpn/menu/xray/trojan_delete.sh "${repoDir}files/menu/xray/trojan_delete.sh"
wget -O /boyevpn/menu/xray/trojan_extend.sh "${repoDir}files/menu/xray/trojan_extend.sh"
wget -O /boyevpn/menu/xray/trojan_detail.sh "${repoDir}files/menu/xray/trojan_detail.sh"
wget -O /boyevpn/menu/xray/trojan_list.sh "${repoDir}files/menu/xray/trojan_list.sh"
wget -O /boyevpn/menu/xray/trojan_monitor.sh "${repoDir}files/menu/xray/trojan_monitor.sh"
wget -O /boyevpn/menu/xray/vless_add.sh "${repoDir}files/menu/xray/vless_add.sh"
wget -O /boyevpn/menu/xray/vless_delete.sh "${repoDir}files/menu/xray/vless_delete.sh"
wget -O /boyevpn/menu/xray/vless_extend.sh "${repoDir}files/menu/xray/vless_extend.sh"
wget -O /boyevpn/menu/xray/vless_detail.sh "${repoDir}files/menu/xray/vless_detail.sh"
wget -O /boyevpn/menu/xray/vless_list.sh "${repoDir}files/menu/xray/vless_list.sh"
wget -O /boyevpn/menu/xray/vless_monitor.sh "${repoDir}files/menu/xray/vless_monitor.sh"
wget -O /boyevpn/menu/xray/vmess_add.sh "${repoDir}files/menu/xray/vmess_add.sh"
wget -O /boyevpn/menu/xray/vmess_delete.sh "${repoDir}files/menu/xray/vmess_delete.sh"
wget -O /boyevpn/menu/xray/vmess_extend.sh "${repoDir}files/menu/xray/vmess_extend.sh"
wget -O /boyevpn/menu/xray/vmess_detail.sh "${repoDir}files/menu/xray/vmess_detail.sh"
wget -O /boyevpn/menu/xray/vmess_list.sh "${repoDir}files/menu/xray/vmess_list.sh"
wget -O /boyevpn/menu/xray/vmess_monitor.sh "${repoDir}files/menu/xray/vmess_monitor.sh"
wget -O /boyevpn/menu/other/check.sh "${repoDir}files/menu/other/check.sh"
wget -O /boyevpn/menu/other/update_script.sh "${repoDir}files/menu/other/update_script.sh"
wget -O /boyevpn/menu/other/update_package.sh "${repoDir}files/menu/other/update_package.sh"
wget -O /boyevpn/menu/other/web_login.sh "${repoDir}files/menu/other/web_login.sh"
find /boyevpn/menu/ -type f -exec chmod +x {} \;
...
echo "{
    ...
    \"wireguard\" : []
}" > /boyevpn/user_database.json

# Configure cron
echo "[INFO] Configure cron"
sleep 2
mkdir /boyevpn/cron
wget -O /boyevpn/cron/reboot.sh "${repoDir}files/cron/reboot.sh"
wget -O /boyevpn/cron/hourly.sh "${repoDir}files/cron/hourly.sh"
wget -O /boyevpn/cron/midnight.sh "${repoDir}files/cron/midnight.sh"
sed -i "s/install_domain/${installSubDomain}/g" /boyevpn/cron/reboot.sh
find /boyevpn/cron/ -type f -exec chmod +x {} \;
(crontab -l; echo "@reboot /boyevpn/cron/reboot.sh") | crontab -
(crontab -l; echo "@hourly /boyevpn/cron/hourly.sh") | crontab -
(crontab -l; echo "@midnight /boyevpn/cron/midnight.sh") | crontab -
