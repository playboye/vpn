...
clear
if [[ ! $(cat /boyevpn/user_database.json | jq -r '.ssh[].username') ]]; then
	echo ""
...
password=$(cat /boyevpn/user_database.json | jq -r '(.ssh[] | select(.username == "'$username'")).password')
expired=$(cat /boyevpn/user_database.json | jq -r '(.ssh[] | select(.username == "'$username'")).expired')
...
echo "Public Key  : $(cat /boyevpn/dnstt/server.pub)"
echo ""
