...
uuid=$(cat /boyevpn/user_database.json | jq -r '.xray.vless[] | select(.username == "'$username'").uuid')
expired=$(cat /boyevpn/user_database.json | jq -r '.xray.vless[] | select(.username == "'$username'").expired')
