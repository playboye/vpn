...
if [[ ! $(cat /boyevpn/user_database.json | jq -r '.xray.trojan[].username') ]]; then
	echo ""
...
clear
echo ""
for username in $(cat /boyevpn/user_database.json | jq -r '.xray.trojan[].username'); do
	n=$((n+1))
	user[$n]=$username
	echo "[$n] $username"
done
...
exp_old=$(cat /boyevpn/user_database.json | jq -r '(.xray.trojan[] | select(.username == "'$username'")).expired')
diff=$((($(date -d "${exp_old}" +%s)-$(date +%s))/(86400)))
duration=$(expr $diff + $days + 1)
exp_new=$(date -d +${duration}days +%Y-%m-%d)

cat /boyevpn/user_database.json | jq '(.xray.trojan[] | select(.username == "'$username'")).expired |= "'$exp_new'"' > /boyevpn/user_database.json.tmp
