...
password=$(cat /boyevpn/user_database.json | jq -r '.xray.trojan[] | select(.username == "'$username'").password')
expired=$(cat /boyevpn/user_database.json | jq -r '.xray.trojan[] | select(.username == "'$username'").expired')
