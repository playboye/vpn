...
exp_old=$(cat /boyevpn/user_database.json | jq -r '(.xray.vmess[] | select(.username == "'$username'")).expired')
diff=$((($(date -d "${exp_old}" +%s)-$(date +%s))/(86400)))
duration=$(expr $diff + $days + 1)
exp_new=$(date -d +${duration}days +%Y-%m-%d)

cat /boyevpn/user_database.json | jq '(.xray.vmess[] | select(.username == "'$username'")).expired |= "'$exp_new'"' > /boyevpn/user_database.json.tmp
