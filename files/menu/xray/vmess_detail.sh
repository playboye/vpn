...
uuid=$(cat /boyevpn/user_database.json | jq -r '.xray.vmess[] | select(.username == "'$username'").uuid')
expired=$(cat /boyevpn/user_database.json | jq -r '.xray.vmess[] | select(.username == "'$username'").expired')
