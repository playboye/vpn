...
clear
echo ""
until [[ ! -z $username ]] && [[ ! $(cat /boyevpn/user_database.json | jq -r '(.xray.vmess[] | select(.username == "'$username'")).username') ]] && [[ $username == ?(+|-)+([a-zA-Z0-9]) ]]; do
	read -p "Username : " username
	[[ -z $username ]] && echo -e "[ERROR] Null input."
	[[ $(cat /boyevpn/user_database.json | jq -r '(.xray.vmess[] | select(.username == "'$username'")).username') ]] && echo -e "[ERROR] User already exist."
...
uuid=$(xray uuid)
while cat /boyevpn/user_database.json | jq -r '.xray.vmess[].uuid' | grep -qw "$uuid"; do
    uuid=$(xray uuid)
done
...
cat /boyevpn/user_database.json | jq '.xray.vmess += [{"username": "'${username}'","uuid": "'${uuid}'","expired": "'${expired}'"}]' > /boyevpn/user_database.json.tmp
